...
export const systemRouter = createTRPCRouter({
  getStatus: protectedProcedure.query(async ({ ctx }) => {
    ...
    // Check Gemini AI health (we'll assume it's online if we have the API key)
    ...
    const [
      ...
      ctx.prisma.opportunity.count({ where: { createdAt: { gte: last24Hours } } })
    ]);

    // Get AI session metrics
    const aiSessions = await ctx.prisma.aIAnalysisSession.findMany({
      where: {
    ...
    const currentJobs = await ctx.prisma.aIAnalysisSession.findMany({
      where: {
        OR: [
    ...
    const subredditCursors = await ctx.prisma.subredditCursor.findMany({
    ...
    return {
      systemHealth: {
        ...
        inngest: 'online' as const, // We assume Inngest is online if we're running
        reddit: redditHealth,
        gemini: geminiHealth,
      ...
      currentJobs: currentJobs.map(job => ({
        ...
        status: job.endTime ? 'completed' as const : 'running' as const,
      ...
      recentActivity: subredditCursors.map(cursor => ({
        ...
        lastUpdate: cursor.updatedAt.toISOString()
      }))
    };
  }),
});